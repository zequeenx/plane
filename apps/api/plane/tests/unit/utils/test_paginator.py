...
class TestPaginateGroupByValidation:
    ...
    @pytest.mark.parametrize("prefix", ["customproperty_", "modulecustomproperty_"])
    def test_valid_annotated_custom_group_by_passes_through(self, prefix):
        field_name = f"{prefix}00000000-0000-4000-8000-000000000001"
        queryset = SimpleNamespace(query=SimpleNamespace(annotations={field_name: object()}))
        request = _make_request(group_by=field_name)
        ...
        assert response.data["grouped_by"] == field_name

    @pytest.mark.parametrize("prefix", ["customproperty_", "modulecustomproperty_"])
    def test_unannotated_custom_group_by_raises_parse_error(self, prefix):
        field_name = f"{prefix}00000000-0000-4000-8000-000000000001"
        queryset = SimpleNamespace(query=SimpleNamespace(annotations={}))
        request = _make_request(group_by=field_name)
